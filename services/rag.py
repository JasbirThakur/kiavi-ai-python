...
5. Diagrams & Figures: If the Knowledge section contains an image tag or diagram (e.g. ![Figure Caption](image_url)), YOU MUST INCLUDE THAT EXACT IMAGE TAG ![Figure Caption](image_url) in your answer so the user can visually see the diagram. NEVER make up or hallucinate fake URLs (like imgur, placeholder, or example.com). If the user asks for a diagram, visual workflow, or architecture and no image URL exists in the knowledge, draw a clean structured ASCII flowchart or markdown table to visually illustrate the process!
...
def generate_llm_response(messages: list) -> tuple[str, str]:
    # 1. Tier 1: NVIDIA NIM
    # Priority 1: nvidia/nemotron-3-embed-1b
    # Fallback Priority 2: nvidia/nemotron-3.5-lightning-30b-a3b (Fast 1.2s response)
    if nvidia_client:
        models_to_try = [
